    ...
    for line in lines:
        ...
        if line.startswith("Account Number:"):
            ...
            if "username" not in current:
                ...
                # Legacy account created before the username
                # feature existed. Fall back to the account
                # number as its username so it can still log in.
    ...
    with open(
        ...
        for saved_account in accounts:
            ...
            if (
                ...
                # This is the account that changed. Use the
                # in-memory object directly so any updated
                # balance, username, or PIN gets written out.

                current_account = account

            else:

                current_account = saved_account
            ...
            file.write(
                f"PIN: "
                f"{current_account.get_pin()}\n"
            )

            file.write(
                f"Account Type: "
                f"{current_account.get_account_type()}\n"
            )

            file.write(
                f"Balance: "
                f"{current_account.check_balance():.2f}\n\n"
            )